  ...
  return (
    <nav className="w-full bg-white shadow-lg z-50">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 flex items-center justify-between h-16">
        {/* Logo */}
        <Link href={routes.welcome()} className="flex items-center">
          <EatWebLogo />
        </Link>

        {/* Desktop links */}
        <div className="hidden lg:flex lg:space-x-8">
          <NavLink href={routes.companyProducts()} label="Продукты" pathname={pathname} />
          <NavLink href={routes.orders()} label="Заказы" pathname={pathname} />
          <NavLink href={routes.companyAccount()} label="Профиль" pathname={pathname} />
        </div>
      ...
      {/* Mobile slide-in */}
      <MobileNav show={showMobileNav} pathname={pathname} onClose={() => setShowMobileNav(false)} />
    </nav>
  )
}
